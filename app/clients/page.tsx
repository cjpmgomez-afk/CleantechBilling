...
export default function Clients() {
  ...
  return (<div>
    ...
    <div className="card overflow-auto"><table className="table">
      ...
      <tbody>{rows.map((c: any) => <tr key={c.id}><td>{c.name}</td><td>{c.phone ?? "—"}</td><td>{c.pppoeProfile ?? "—"}</td><td>{c.speed ?? "—"}</td><td>₱{c.monthlyFee}</td><td><span className="badge">{c.status}</span></td><td><button className="badge" onClick={() => toggle(c)}>{c.status === "ACTIVE" ? "Disconnect" : "Activate"}</button></td></tr>)}</tbody>
    </table></div>
